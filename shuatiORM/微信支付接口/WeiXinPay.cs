...
namespace Test
{
    class Program
    {
        ...
        {
            WeiXinPay wxpay = new WeiXinPay();//创建对象
            string url = wxpay.GetPayUrl("123455234");//把订单号给微信接口返回url

            string path = wxpay.StringToImagePath("1.png");//根据url生成二维码
            Console.WriteLine(path);//获取二维码图片的存放路径
    ...
    public class WeiXinPay
    {
        ...
        public string GetPayUrl(string productId)
        {
            ...
            data.SetValue("body", "sdasd");//商品描述,不能为空
            data.SetValue("attach", "");//附加数据,可以为空
            data.SetValue("out_trade_no", WxPayApi.GenerateOutTradeNo());//随机字符串 ,这个是你的订单号
            data.SetValue("total_fee", 17500);//总金额,不能有小数,单位是分,1是代表1分钱
            data.SetValue("time_start", DateTime.Now.ToString("yyyyMMddHHmmss"));//交易起始时间
            data.SetValue("time_expire", DateTime.Now.AddMinutes(10).ToString("yyyyMMddHHmmss"));//交易结束时间
            data.SetValue("goods_tag", "");//商品标记,可以为空
    ...
    public class WxPayConfig
    {
        ...
        */
        public const string NOTIFY_URL = "http://www.no5.com.cn/填写自己的路径/Notify.aspx";
