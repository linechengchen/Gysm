...
public partial class BasePageShuati : System.Web.UI.Page
{
        ...
        where MyTable : ITableImplement, new()
    {
        ...
        foreach (AttributeItem attr1 in list1)
        {
            ...
            string f1 = Request[pre1 + attr1.FieldName];//Request.QueryString取不到submite的参数
            if (isNotContainsZero && f1 == "0")
                f1 = "";
            if (!string.IsNullOrEmpty(f1))
            {
                ...
                {
                    baseErrMsg += f1 + "-转换异常：" + ex1.Message;
                }
            }
            else
